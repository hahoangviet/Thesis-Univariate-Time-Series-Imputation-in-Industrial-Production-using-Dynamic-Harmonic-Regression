...
# step 2
tslm_res <- tslm$residuals %>% msts(seasonal.periods=c(48, 336))
# mstl(tslm_res)[,3] (seasonal48), mstl(tslm_res)[,4] (seasonal336)
plot(mstl(tslm_res))
...
adj_dynamics_imp <- msts(adj_dynamics_imp, c(48, 336))
...
# step 4: add seasonal imputation to dynamics imputation to get result
